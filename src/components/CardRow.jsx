  ...
  return (
    <div ref={ref} className="px-12 py-7">
      ...
      <div className="flex gap-3 overflow-x-auto pb-3" style={{ scrollbarWidth: 'none' }}>
        {cards.map((card, i) => {
          const videoSrc = `/${card.videoKey.replace('v', 'video')}.mp4`;
          return (
            <motion.div
              key={i}
              ...
              className="relative flex-shrink-0 w-64 h-36 rounded-lg overflow-hidden cursor-pointer group shadow-lg"
              onMouseEnter={(e) => {
            ...
            >
              {/* Show the video directly instead of an image */}
              <video 
                src={videoSrc}
                muted 
                loop 
              ...
              {/* Hover overlay with play button and label */}
              <motion.div
                initial={{ opacity: 0 }}
              ...
              >
                ...
                <span className="text-white text-xs font-semibold text-center px-2 drop-shadow-md">{card.label}</span>
              </motion.div>
